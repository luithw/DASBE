...
def clustering(x, bestnet, labels, cortical_delay, iterations,refractory):
    ...
    for iter in range(iterations):
        ...
        for t in range(cortical_delay):
            ...
            # encoding_record[-1].append(np.array(encoding.reshape(10,20).cpu().detach()))
            spike_record[-1].append(np.array(s.cpu()))
            context_record[-1].append(np.array(output.reshape(W,H).detach().cpu()))
            encoding_record[-1].append(np.array(encoding.reshape(40, 40).detach().cpu()))
    ...
    return spike_record, encoding_record, sil_record, rate_record, color_record
...
for seed in seed_list:
    ...
    spk, enc, syn, r, c = clustering(np.array(multi[i], dtype=np.float32), net,multi_label[i],cortical_delay=32,iterations=30,refractory=9)
    SPK.append(spk)
    SYN.append(syn)
    RATE.append(r)
    COLOR.append(c)
    ENCODING.append(enc)
    groups,selected = labeled_synchrony_measure(spk,multi_label[i], show=False)
    ...
    # pred_high = k_means_var(enc,multi_label[i], K = [4], back=10, smooth=1)
